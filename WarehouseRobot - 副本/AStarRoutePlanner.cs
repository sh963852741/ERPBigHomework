...
        private readonly ICostGetter costGetter = new SimpleCostGetter();
        ...
        public IList<RHPoint> Plan(RHPoint start, RHPoint destination)
        {
            ...
            AStarNode startNode = new(start.ToPoint(), null, 0, 0);
...
            return DoPlan(routePlanData, currenNode);
        ...
        private IList<RHPoint> DoPlan(RoutePlanData routePlanData, AStarNode currenNode)
        {
            ...
            foreach (CompassDirections direction in allCompassDirections)
            {
                ...
                if (costH == 0)
                {
                    ...
                    IList<RHPoint> route = new List<RHPoint>();
                    route.Add(routePlanData.Destination);
                    route.Insert(0, new RHPoint(currenNode.Location));
                    AStarNode tempNode = currenNode;
                    while (tempNode.PreviousNode != null)
                    {
                        route.Insert(0, new RHPoint(tempNode.PreviousNode.Location));
